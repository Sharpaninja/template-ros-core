...
def solution(obs):
    ...
    while True:
        ...
        if len(cntRazmetkaY)>=2:
            ...
            raschet = 1
            #else:
            #    xRazmetkaYpred = centrX-int(centrX/3)
                #xl = int(X-(X/6))
            #yl = 0
            #xWhiteline = xWhitelinepred
            #if len(cntWhiteline)>0:
            #    for i in range (len(cntWhiteline)):
            #        if cv2.contourArea (cntWhiteline[i]) > 200:
            #            Mi = cv2.moments(cntWhiteline[i])
            #            xi = int (Mi [ 'm10' ] / Mi [ 'm00' ])
            #            yi = int (Mi [ 'm01' ] / Mi [ 'm00' ])
            #            if xi > xRazmetkaY and xi<xl and yi>yl:
            #                yl = yi
            #                xl = xi
            #    xWhitelinepred = xl
            #if raschet == 1:
